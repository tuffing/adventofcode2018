...
class Solution(object):
	...
	def solution(self, fileName):
		...
		turn = turn.split(',')
		result = ('%d,%d' % (int(turn[1]), int((turn[0]))))
		print(result)
	...
	def takeTurn(self, players, rows):
		...
		for coords in order:
			...
			if newCoords in players.keys():
				#theres a collision
				del(players[newCoords])
				if newCoords in order:
					order.remove(newCoords)
				continue
		
			if coords[0] >= len(rows) or coords[0] < 0 or coords[1] >= len(rows[coords[0]]) or coords[1] < 0 or rows[coords[0]][coords[1]] == ' ':
				#out of rail
				continue
