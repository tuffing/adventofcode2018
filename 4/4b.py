...
def processGuardsMostSleptAtMinute(guards):
	for id,guard in guards.items():
		findTimeGuardIsMostAsleep(guard)
...
def run():
	...
	print('Guard is #%s who is most asleep %i times at %i multipled that is %i' % (guardAsleep, guard['minuteMostAsleepCount'], guard['minuteMostAsleep'],int(guardAsleep) * guard['minuteMostAsleep']))
