...
class Solution(object):
	...
	def solution(self, serial):
		...
		z = [self.checkGrid(grid, squares, x, y) for y in range(1, 298) for x in range(0, 297)]
	...
	def checkGrid(self, grid, squares, x, y):
		...
		for gridSize in range (1, 301):
			if (x + gridSize > len(grid)-1 or y + gridSize > len(grid )-1):
				break;

			for iy in range(y, y+gridSize):
				for ix in range(x, x+gridSize):
					#print('%d,%d' % (ix,iy))
					total += grid[iy][ix]
